...
ter::window_context::window_context(window_system::window *window,
  ...
  createInfo.setImageExtent(
      vk::Extent2D(window->get_width(), window->get_height()));
  ...
  if (GDK_IS_WAYLAND_WINDOW(gdkWindow)) {
    createInfo.setSurfaceKHR(vk::WaylandSurfaceCreateInfoKHR(
        {}, gdk_wayland_window_get_wl_surface(GDK_WAYLAND_WINDOW(gdkWindow))));
  ...
  // Check if the window is an X11 window
    ...
    createInfo.setSurfaceKHR(vk::XlibSurfaceCreateInfoKHR(
        {}, gdk_x11_window_get_xid(GDK_X11_WINDOW(gdkWindow)), nullptr));
