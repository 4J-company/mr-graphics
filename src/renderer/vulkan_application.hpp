  ...
  class VulkanState {
    ...
    public:
      ...
      void create_pipeline_cache()
      {
        ...
        std::fstream cache_file("bin/cache/pipeline",
        ...
        if (cache_file) {
          ...
          cache_file.open("bin/cache/pipeline",
                          std::ios::out | std::ios::binary);
        }

        // create pipleline cache
        vk::PipelineCacheCreateInfo pipeline_cache_create_info {
          .initialDataSize = cache_size, .pInitialData = cache_data};
        _pipeline_cache =
          _device.createPipelineCache(pipeline_cache_create_info).value;
        ...
        char *cache_save_data =
          new char[cache_size]; // TODO: maybe using cache_data buffer instead
        result = _device.getPipelineCacheData(
          _pipeline_cache, &cache_size, cache_save_data);
        assert(result == vk::Result::eSuccess);
