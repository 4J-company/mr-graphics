...
void mr::Pipeline::create_layout_sets(const VulkanState &state, std::vector<std::vector<vk::DescriptorSetLayoutBinding>> bindings)
{
  ...
  for (int i = 0; i < bindings.size(); i++)
  {
    ...
    _set_layouts[i] = state.device().createDescriptorSetLayoutUnique(set_layout_create_info).value;
  }
