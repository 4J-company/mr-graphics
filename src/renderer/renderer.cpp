...
ter::application::application()
{
  ...
  vk::PhysicalDevice phys_device = _instance.enumeratePhysicalDevices().front();

  // get the QueueFamilyProperties of the first PhysicalDevice
  std::vector<vk::QueueFamilyProperties> queueFamilyProperties = phys_device.getQueueFamilyProperties();
  ...
  vk::DeviceQueueCreateInfo device_queue_ci(vk::DeviceQueueCreateFlags(), static_cast<uint32_t>( graphicsQueueFamilyIndex ), 1, &queuePriority);
  _device = phys_device.createDevice(vk::DeviceCreateInfo( vk::DeviceCreateFlags(), device_queue_ci));
